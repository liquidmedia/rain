class Rain::CloudsController < ApplicationController
  include ApplicationHelper
  include RainCmsHelper
  
  before_filter :drop, :except => [:create]
  before_filter :track_referrer
  ...
  def cloud    
    ...
    render '/rain/clouds/cloud', :locals=>{:cloud=>@drop}, :layout => request.xhr? ? '' : @drop.layout
  end
  ...
  def create
   ...
   def update
     respond_to do |format|
       if @drop.update_attributes(params[:rain_cloud])
         flash[:success] = t(:drop_updated)
         format.html { redirect_to "/#{@drop.name}" }
         format.xml  { head :ok }
         format.js   { render '/rain/drops/update', :layout=>false}
       else
  ...
  private
    def drop
      ...
      if name.to_i > 0
        ...
        @drop ||= Rain::Drop.find_by_name(name) || Rain::Cloud.new(:name=>name)
      end
